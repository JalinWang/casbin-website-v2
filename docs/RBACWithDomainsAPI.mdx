...
> Note: Conventionally domain token name in policy definition is `dom` and placed in the beginning. Now Golang version supports customized token name & place. If the subject token name is `dom`, the subject token can be placed at an arbitrary place and no extra action needs.  If the subject token name is other than `dom` , `e.SetFieldIndex()` for `constant.DomainIndex` should be called after the enforcer is initialized.
>
> ```ini 
> # `domain` here for `dom`
> [policy_definition]
